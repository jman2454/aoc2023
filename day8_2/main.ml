...
let get_next node c map = 
  let (l, r) = StringMap.find node map in
...
let rec find_zs buf map count node = 
  if node.[String.length node - 1] = 'Z' then 
    count 
  else
    let c = Buffer.get_value buf in 
    find_zs (Buffer.next buf) map (count + 1) (get_next node c map)

let rec gcd a b = if b = 0 then a else gcd b (a mod b)
let get_lcm a b = (a / (gcd a b)) * b
